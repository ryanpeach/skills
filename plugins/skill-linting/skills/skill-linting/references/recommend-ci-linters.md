...
If your repo just contains skills, `skill-validator check` does both roles at once.
...
And follow the example at [./references/lint.yml](./references/lint.yml)

## Pre-commit

Follow the example at [./references/.pre-commit-config.yaml](./references/.pre-commit-config.yaml)