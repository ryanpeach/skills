...
1. `fork` is the branch that acts as your fork's default branch. It's where you keep work that is specific to your fork, and can contain commits from both your `fork/*` and `feature/*` branches. It can also be rebased on top of upstream changes from the original repo's default branch as needed.
...
The `fork` branch is the case worth calling out specifically: always treat it as a **future-main** branch. It is what upstream's default branch *will become* once all of your in-flight `feature/*` branches have been merged in. In other words, `fork` is meant to sit *slightly ahead of* `main` — it carries upstream plus your pending contributions, so it's the natural working state for the fork while still staying cleanly rebasable onto upstream.
